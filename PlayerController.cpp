...
{
	player->State = Keyboard::GetInstance()->GIsKeyUp(Dik_LEFT) && Keyboard::GetInstance()->GIsKeyUp(DIK_RIGHT) ? Object::Standing : Object::Running;
	player->SetBound(16, 35);

	if (Keyboard::GetInstance()->IsKeyDown(Dik_JUMP))
	{
		player->StartJump(-Gravity);
		return;
	}

	if (Keyboard::GetInstance()->IsKeyDown(Dik_DOWN) && Keyboard::GetInstance()->GIsKeyUp(Dik_LEFT) && Keyboard::GetInstance()->GIsKeyUp(DIK_RIGHT))
	{
		SitState();
		return;
	}
...
void PlayerController::SitState()
{
	...
	if (Keyboard::GetInstance()->GIsKeyUp(Dik_DOWN))
	{
		StandState();
		return;
	}
...
void PlayerController::ArrowState()
{
	...
	bool keyRun = Keyboard::GetInstance()->IsKeyDown(Dik_LEFT) || Keyboard::GetInstance()->IsKeyDown(Dik_RIGHT);
	player->AngleGun = 0;
...
void PlayerController::AttackState()
{
	...
	//
	if (Keyboard::GetInstance()->GIsKeyUp(Dik_ATTACK) || player->GunType == OItem::M)
	{
		//Tạo Đạn Theo Súng
		isAllowAttack = true;
	}
	...
	if (isAllowAttack && Keyboard::GetInstance()->IsKeyDown(Dik_ATTACK) && (timeReload > timeR - 0.05))
	{
		timeReload = 0;
		isAllowAttack = false;
